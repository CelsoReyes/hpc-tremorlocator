...
! Opening of in-file
      ...
      open(fid_bin_in, file= (trim(inname) // ".bin"),access="direct", &
              STATUS="OLD",RECL=(nLon*nLat*nDepth*4), action="read", iostat=fileiostat)
...
! Close the v5d file (absolutely mandatory) and exit
  ...
  CONTAINS ! ###############################################################################
    ...
    SUBROUTINE get_header()    
      OPEN(fid_header, FILE=(trim(inname) // ".hdr"), iostat= fileiostat, &
      status = "old", ACTION="READ")
      ! read header info equivelent to:       nr, nc, nl, numtimes, numvars
      READ(fid_header, '(I10)') nLon, nLat, nDepth, numtimes !Header Information
      READ(fid_header, '(F8.5)') nfreq
      READ(fid_header, '(I10)') numvars !numvars, number of variables in data set
      ! read header info equivelent to: starttime
      READ(fid_header, '(I5)') curdate  ! YYDDD
      READ(fid_header, '(I6)') curtime  ! HHMMSS
         
      READ(fid_header, '(F10.5)') northmost_point, westmost_point !Northmost, WestMost
      READ(fid_header, '(F10.5)') row_spacing, column_spacing, vert_spacing
      CLOSE(fid_header)
